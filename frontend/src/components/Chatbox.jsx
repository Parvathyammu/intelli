  ...
  const handleSendMessage = async () => {
    ...
    try {
      ...
      const botMessage = {
        ...
        text: data.answer || "Sorry, I couldn't find anything relevant.",
        sender: 'bot',
        timestamp: new Date()
      };
...
const formatText = (text) => {
  ...
  const boldProcessed = text.replace(/\*\*(.*?)\*\*/g, (_, boldText) => {
    return `@@BOLD@@${boldText}@@END@@`;
  });

  // Step 2: Replace single * with newline
  const newlineProcessed = boldProcessed.replace(/\*/g, "");

  // Step 3: Remove any remaining stray *
  const cleaned = newlineProcessed.replace(/\*/g, "");
  ...
  return lines.map((line, index) => {
    const parts = line.split(/@@BOLD@@(.*?)@@END@@/g);
      ...
      >
        {messages.map((message) => (
          ...
          >
            ...
            >
              <p className="text-sm">{formatText(message.text)}</p>
